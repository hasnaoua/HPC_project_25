...
float compute_loss(MLP *m, float *X, int *y, int N, float reg_lambda) {
    ...
    float loss = 0.0f; // safe initialization
    ...
    if (!z1 || !a1 || !z2 || !probs) {
        // allocation failed, ensure safe return
        loss = INFINITY;
        goto cleanup;
    }
    ...
    for (int i = 0; i < N; i++)
        loss += -logf(fmaxf(probs[i * m->n_out + y[i]], 1e-12f));
...
#pragma omp parallel for
    for (int i = 0; i < L1; i++) m->W1[i] -= lr * dW1[i];
#pragma omp parallel for
    for (int i = 0; i < L2; i++) m->W2[i] -= lr * dW2[i];
#pragma omp parallel for
    for (int i = 0; i < m->n_hidden; i++) m->b1[i] -= lr * db1[i];
#pragma omp parallel for
    for (int i = 0; i < m->n_out; i++) m->b2[i] -= lr * db2[i];
#else
    for (int i = 0; i < L1; i++) m->W1[i] -= lr * dW1[i];
    for (int i = 0; i < L2; i++) m->W2[i] -= lr * dW2[i];
    for (int i = 0; i < m->n_hidden; i++) m->b1[i] -= lr * db1[i];
...
void train(MLP *m, float *X, int *y, int N,
    ...
    for (int epoch=0; epoch<num_passes; ++epoch) {
        ...
        for (int b=0; b<num_batches; ++b) {
            ...
            // Copy batch
            for (int i=0;i<bs;i++) {
                memcpy(&Xb[i*m->n_in], &X[(start+i)*m->n_in], m->n_in*sizeof(float));
                yb[i] = y[start+i];
            }
            ...
            for(int i=0;i<L1;i++) dW1[i] = dW1[i]*inv_bs + reg_lambda*m->W1[i];
            for(int i=0;i<L2;i++) dW2[i] = dW2[i]*inv_bs + reg_lambda*m->W2[i];
        ...
        if(print_loss && (epoch%100==0 || epoch==num_passes-1)) {
            ...
            else global_L = local_L;

            if(world_rank==0) {
                if(world_size>1) global_L/= (float)world_size;
